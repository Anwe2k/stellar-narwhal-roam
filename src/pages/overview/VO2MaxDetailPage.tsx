...
import { Wind, Timer, Heart, Activity } from 'lucide-react';
...
const VO2MaxDetailPage = () => {
  ...
  const calculateRatio = () => {
    ...
    if (r === 0 || isNaN(m) || isNaN(r)) return 0;
  ...
  const calculateRockport = () => {
    ...
    if (isNaN(timeTotal) || isNaN(hr) || timeTotal === 0) return 0;
  ...
  return (
    <MobileLayout title="VO2 Max" headerGradientClass="from-[#FFDAD6]/40" backPath="/overview/vitals">
            ...
            {logs.length > 0 ? (
              <div className="h-40 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={logs}>
                    <XAxis dataKey="date" stroke="#9CA3AF" fontSize={10} axisLine={false} tickLine={false} tickFormatter={(val) => formatDate(val)} />
                        ...
                        if (active && payload && payload.length) {
                          return (
                            <div className="bg-white p-2.5 rounded-xl shadow-md border border-gray-100 text-xs">
                              ...
                              <p className="text-gray-400">{formatDate(payload[0].payload.date)}</p>
                            </div>
                          );
                        }
                        return null;
              ...
              <div className="h-32 w-full flex items-center justify-center bg-gray-50 rounded-2xl">
                <span className="text-xs text-gray-400 font-medium">No recorded graphs. Add below.</span>
              </div>
            )}
          </CardContent>
        </Card>
